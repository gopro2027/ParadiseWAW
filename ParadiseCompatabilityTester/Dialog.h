...
OutputInfo.numCharsResultString = 500;
...
if(oskdialog_mode == MODE_OPEN)
 ...
 if( oskdialog_mode == MODE_CLOSE )
 {
        ...
        makekbStr((char*)(*(&OutputInfo.pResultString)),dest, strLen-1/*-1 because shit iz fucked up dawg*/);
        inputFieldInfo.init_text = (uint16_t*)INIT_TEXT;      
        if(cellSysutilUnregisterCallback(0) != 0) {printf("\nCouldn't unload the keyboard !\n");}
        oskdialog_mode = MODE_EXIT;
		printf("Exited\n");
 }
